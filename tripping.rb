...
def available_ranges(property)
    ...
    # availability = availability[0..29]
    puts "availability: "
    # print availability
    puts "***"
    ...
    for i in (0..availability.length)
        ...
        if availability[i] === "Y"
         ...
         # If availability[i] is N, this is not a possible date range. Reset counting variables.
        # if availability[i] === "N"
        #     # continue
        # end
        # 
    ...
    return available_dates
        ...
        # possible_start_index = nil
    # If i == possible_start_index + min_days
        # add the date corresponding to availability[possible_start_index] and the date correspondning to availability[i] to the output

end


# TODO: figure out how to get overlapping date ranges




# Once it hits a Y, set start index and min stay and do a 2nd for loop to continue iterating through for at least the length of min stay
# After hitting min stay, add each additional Y to output array. Keep going until you hit a N, then go back and do this for each Y you passed

# Use recursion?

# def find_available_ranges(property, start_date=0, min_stay=0)
